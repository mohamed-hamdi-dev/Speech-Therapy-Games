...
const Result = () => {
  ...
  const { endTherapistSession, therapistSession } = useTherapyStore();
      ...
      <div className="bg-white rounded-[3rem] border border-[#f0dda7] p-8 md:p-10 text-center shadow-xl shadow-amber-100/40">
        ...
        {sessionData && (
          <div className="grid md:grid-cols-3 gap-4 mb-8 text-right">
            <div className="rounded-[2rem] bg-[#eff6ff] p-5 border border-blue-100">
              <div className="text-sm font-bold text-blue-700 mb-2">Score</div>
            ...
            <div className="rounded-[2rem] bg-[#fff7e7] p-5 border border-amber-100">
              <div className="text-sm font-bold text-amber-700 mb-2">نوع الجلسة</div>
              <div className="text-2xl font-black text-slate-900">
                {sessionData.therapistMode ? 'جلسة عيادة' : 'جلسة منزلية'}
              </div>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          ...
          {therapistSession?.isActive && (
            <Button
              variant="outline"
              className="text-xl !rounded-[1.6rem] !py-4 px-10 bg-red-50 border-red-100 text-red-600"
              onClick={() => {
                endTherapistSession();
